...
def predict_weather(interval):
    ...
    with ZipFile('./modelos/model__temp.p.zip', 'r') as myzip:
        myzip.extractall('./')

    with ZipFile('./modelos/model__hum.p.zip', 'r') as myzip:
        myzip.extractall('./')

    arima_temp = pickle.load( open( './modelos/model__temp.p', "rb" ) )
    predicc_temp, confint = arima_temp.predict(n_periods=interval, return_conf_int=True)

    aria_hum = pickle.load( open( './modelos/model__hum.p', "rb" ) )
    predicc_hum, confint2 = aria_hum.predict(n_periods=interval, return_conf_int=True)
...
@app.route("/servicio/v1/prediccion/test",methods=['GET'])
def test():
    response = Response("Test Api V1", status=200)
